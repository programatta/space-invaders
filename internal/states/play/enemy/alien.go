...
func NewAlien(posX, posY float32, sprite1, sprite2 sprite.Sprite, score uint8, alienMoveDelay float32, notifier common.Notifier) *Alien {
	...
	return &Alien{sprites: sprites, posX: posX, posY: posY, score: score, alienMoveDelay: alienMoveDelay, currentSprite: 0, time: 0, notifier: notifier, remove: false, currentDelay: alienMoveDelay}
}
...
func (a *Alien) Update() {
	...
	if a.time >= a.currentDelay {
		a.posX += config.Speed * config.Dt * a.currentDirX
...
func (a *Alien) Draw(screen *ebiten.Image) {
	...
	screen.DrawImage(a.sprites[a.currentSprite].Image, opAlien)
}
